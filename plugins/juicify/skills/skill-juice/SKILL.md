...
| **적용 가설** | 현재 프로젝트/작업에 적용 가능한 패턴 가설. 각 가설에 검증 포인트 포함 |
...
## 적용 가설 (검증 전)
(현재 맥락에서 적용 가능한 패턴 가설. 각 가설에 "검증 포인트" 포함)
```
...
## 6. 적용 가설 (검증 전)
(적용 가능한 패턴 가설, 기존 스킬과의 비교. 각 가설에 검증 포인트 필수)
```
...
- **"적용 가설"은 검증 가능하게**: "참고할 만하다" 수준이 아니라, 구체적 적용 제안 + **검증 포인트**를 함께 쓴다. 검증 포인트란 "이 제안이 유효하려면 현재 프로젝트에서 ~를 확인해야 한다"는 전제 조건이다. 적용 제안은 프로젝트 상태를 직접 확인하지 않은 **가설**이므로, 실제 반영 전에 검증이 필요하다는 점을 명시한다.
